...
To explain how this works as simply as possible, a linear regression line is the line that fits best through the observed points when you plot the points of your IVxDV. "Best Fit" in an OLS regression model is defined as the line which produces the smallest value when you take the sum of the squared distances between the estimation line and the observed points (residuals). The idea is that if you need to estimate a Y value that corresponds to a particular X value, the regression line would give you a safe theoretical base for estimation.
