...
The more things you can accomplish within the tidyverse of r packages, the better (IMO). Using dply to produce your summary stats enables you to continue the code seamlessly into the next task (filtering, plotting, etc...).
...
Here, we can get the total number of cars with each class & cyl combination using **group_by()**, **summarize()**, and **spread()**.
```{r, echo=TRUE, message=FALSE, warning=FALSE}
mpg%>%
  group_by(class, cyl)%>%
  summarize(n=n())%>%
...
The table frequency can also be called by using the ftable() command. command.
```{r, echo=TRUE, include=TRUE, eval=TRUE}
