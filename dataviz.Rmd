...
The "jitter" position adds some random noise to each point, which is especially useful when there might be many points overlapping eachother which would be otherwise hidden.

**Example(Left)**: # of cylinders by # of city miles without the "jitter" positioning.
```{r, echo=TRUE, eval=FALSE}
nojitter<- ggplot()+
  geom_point(data=mpg, aes(x=cyl, y=cty))
```

**Example(Right)**: # of cylinders by # of city miles with "jitter" positioning. *Notice how many points are now visible.*
```{r, echo=TRUE, eval=FALSE}
jitter<- ggplot()+
  geom_point(data=mpg, aes(x=cyl, y=cty),position="jitter")
...
nojitter<- ggplot()+
  geom_point(data=mpg, aes(x=cyl, y=cty),size=3)

jitter<- ggplot()+
  geom_point(data=mpg, aes(x=cyl, y=cty),size=3,position="jitter")
...
```{r}
ggplot()+
  geom_point(data=mpg, aes(x=cyl, y=cty, shape=drv, color=drv),size=3,position="jitter")
```
